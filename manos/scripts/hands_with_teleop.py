...
class modes:
    ...
    def teleop(self):
        ...
        #Execute the teleop for mapping
        uuid = roslaunch.rlutil.get_or_generate_uuid(None, False)
        roslaunch.configure_logging(uuid)
        file_path=FILE_PATH_TELEOP
        launch = roslaunch.parent.ROSLaunchParent(uuid, [file_path])
        launch.start()
        ...
        with keyboard.Events() as events:
            for event in events:
                #Shutdow the teleop for mapping with the button 'esc' in the keyboard
                if (event.key == keyboard.Key.esc):
                    launch.shutdown()
                    break
                #Save the Aruco info with the button 'space' in the keyboard
